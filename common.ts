...
import { readableStreamToString, BlueSnapError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new BlueSnapError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new BlueSnapError(e, e.response?.data)
                }
                if (innerError instanceof BlueSnapError) {
                    // Got "BlueSnapError" from the above try block
                    throw innerError;
                }
